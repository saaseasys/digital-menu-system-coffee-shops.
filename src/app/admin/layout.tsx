    ...
    <div className="min-h-screen bg-[#0F0F0F] flex">
      ...
      {sidebarOpen && (
        <div className="lg:hidden fixed inset-0 z-50 bg-black/50" onClick={() => setSidebarOpen(false)}>
          <aside 
            className="absolute left-0 top-0 h-full w-64 bg-[#1A1A1A] border-r border-[#2C1810]"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="p-4 border-b border-[#2C1810] flex justify-between items-center">
              <Link href="/admin/dashboard" className="flex items-center gap-3">
                <Coffee className="w-6 h-6 text-[#D4A574]" />
                <span className="font-bold text-[#D4A574]">Admin Panel</span>
              </Link>
                ...
                return (
                  <Link
                    ...
                    className={`flex items-center gap-3 px-4 py-3 rounded-lg ${
                      isActive ? 'bg-[#D4A574] text-[#0F0F0F]' : 'text-gray-400'
                    }`}
                  >
                    <Icon className="w-5 h-5" />
                    <span>{item.label}</span>
